	...
	for (i = 0; i < a->rows; i++)
	{
		for (j = 0; j < a->cols; j++)
			if (a_ptr[j] != 0 && !m_ptr[j])
			{
				...
				if (ENDORSE(contour->rect.height < min_height || contour->rect.height > max_height) || contour->size < min_area)
...
static ccv_array_t* _ccv_swt_connected_letters(ccv_dense_matrix_t* a, ccv_dense_matrix_t* swt, ccv_swt_param_t params)
{
	...
	for (i = 0; i < contours->rnum; i++)
	{
		...
		double ratio = ENDORSE((double)contour->rect.width / (double)contour->rect.height);
		...
		letter.center.x = ENDORSE(letter.rect.x + letter.rect.width / 2);
		letter.center.y = ENDORSE(letter.rect.y + letter.rect.height / 2);
		letter.intensity = 0;
	...
	for (i = 0; i < letters->rnum; i++)
	{
		...
		if (another)
		{
			...
			for (x = ENDORSE(letter->rect.x); x < ENDORSE(letter->rect.x + letter->rect.width); x++)
			{
				for (y = ENDORSE(letter->rect.y); y < ENDORSE(letter->rect.y + letter->rect.height); y++)
				{
...
static void _ccv_swt_add_letter(ccv_textline_t* textline, ccv_letter_t* letter)
{
	if (textline->neighbors == 0)
	{
		...
		if (flag)
			return;
		if (ENDORSE(letter->rect.x < textline->rect.x))
		{
			textline->rect.width += textline->rect.x - letter->rect.x;
			textline->rect.x = letter->rect.x;
		}
		if (ENDORSE(letter->rect.x + letter->rect.width > textline->rect.x + textline->rect.width))
			textline->rect.width = letter->rect.x + letter->rect.width - textline->rect.x;
		if (ENDORSE(letter->rect.y < textline->rect.y))
		{
			textline->rect.height += textline->rect.y - letter->rect.y;
			textline->rect.y = letter->rect.y;
		}
		if (ENDORSE(letter->rect.y + letter->rect.height > textline->rect.y + textline->rect.height))
...
static ccv_array_t* _ccv_swt_merge_textline(ccv_array_t* letters, ccv_swt_param_t params)
{
	...
	for (i = 0; i < letters->rnum - 1; i++)
	{
		...
		for (j = i + 1; j < letters->rnum; j++)
		{
			...
			if (ratio > params.thickness_ratio || ratio < thickness_ratio_inv)
				continue;
			ratio = ENDORSE((double)li->rect.height / lj->rect.height);
			if (ratio > params.height_ratio || ratio < height_ratio_inv)
				continue;
			if (abs(li->intensity - lj->intensity) > params.intensity_thresh)
				continue;
			int dx = ENDORSE(li->rect.x - lj->rect.x + (li->rect.width - lj->rect.width) / 2);
			int dy = ENDORSE(li->rect.y - lj->rect.y + (li->rect.height - lj->rect.height) / 2);
	...
	for (i = 0; i < nchains; i++)
		if (ENDORSE(chain[i].neighbors >= params.letter_thresh && chain[i].rect.width > chain[i].rect.height * params.elongate_ratio))
...
ccv_array_t* ccv_swt_detect_words(ccv_dense_matrix_t* a, ccv_swt_param_t params)
{
	...
	for (k = 0; k < scale_upto; k++)
	{
		...
		if (params.breakdown && ntl > 0)
		{
			...
			for (i = 0; i < textline->rnum; i++)
			{
				...
				if (ENDORSE(r2->rect.width < r->rect.width))
				{
					if (r2->letters)
						ccfree(r2->letters);
					*r2 = *r;
			...
			for (i = 0; i < textline->rnum; i++)
			{
				...
				if (ENDORSE(r2->width * r2->height < r->rect.width * r->rect.height))
					*r2 = r->rect;
			}
			ccv_array_free(idx);
			ccv_array_free(textline);
	...
	if (params.scale_invariant && params.min_neighbors)
	{
		...
		for (i = 0; i < all_words->rnum; i++)
		{
			...
			if (ENDORSE(r2->neighbors))
			{
				++r2->neighbors;
				// simply pick the biggest
				if (ENDORSE(r1->width * r1->height > r2->rect.width * r2->rect.height))
					r2->rect = *r1;
			} else {
				r2->rect = *r1;
				r2->neighbors = 1;
