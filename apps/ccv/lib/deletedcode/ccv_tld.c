...
static ccv_rect_t _ccv_tld_short_term_track(ccv_dense_matrix_t* a, ccv_dense_matrix_t* b, ccv_rect_t box, ccv_tld_param_t params)
{
	...
	float gapx = (float)ENDORSE(box.width) / TLD_GRID_SPARSITY;
	float gapy = (float)ENDORSE(box.height) / TLD_GRID_SPARSITY;
	float x, y;
	for (x = gapx * 0.5; x < ENDORSE(box.width); x += gapx)
		for (y = gapy * 0.5; y < ENDORSE(box.height); y += gapy)
		{
			ccv_decimal_point_t point = ccv_decimal_point(ENDORSE(box.x) + x, ENDORSE(box.y) + y);
...
static inline float _ccv_tld_rect_intersect(const ccv_rect_t r1, const ccv_rect_t r2)
{
	...
	return (float)intersect / (ENDORSE(r1.width * r1.height + r2.width * r2.height) - intersect);
}
...
static void _ccv_tld_box_percolate_down(ccv_array_t* good, int i)
{
	for (;;)
	{
		...
		if (left < good->rnum)
		{
			...
			if (ENDORSE(left_comp->classification.confidence < smallest_comp->classification.confidence))
				smallest = left;
		}
		if (right < good->rnum)
		{
			ccv_comp_t* right_comp = (ccv_comp_t*)ccv_array_get(good, right);
			if (ENDORSE(right_comp->classification.confidence < smallest_comp->classification.confidence))
				smallest = right;
		}
		if (smallest == i)
			break;
...
static void _ccv_tld_box_percolate_up(ccv_array_t* good, int smallest)
{
	for (;;)
	{
		...
		ccv_comp_t* smallest_comp = (ccv_comp_t*)ccv_array_get(good, smallest);
		if (ENDORSE(smallest_comp->classification.confidence < parent_comp->classification.confidence))
		{
			smallest = parent;
		...
		*(ccv_comp_t*)ccv_array_get(good, one) = c;
		...
		if (other < good->rnum)
		{
			...
			if (ENDORSE(other_comp->classification.confidence < smallest_comp->classification.confidence))
				break;
		}
	}
	...
	for_each_box(comp, input_size.width, input_size.height, params.interval, params.shift, image_size.width, image_size.height)
		...
		if (overlap > params.include_overlap)
		{
			...
			if (agood->rnum < gcap)
			{
				...
				if (overlap > ENDORSE(p->classification.confidence))
				{
...
static void _ccv_tld_ferns_feature_for(ccv_ferns_t* ferns, ccv_dense_matrix_t* a, ccv_comp_t box, uint32_t* fern, dsfmt_t* dsfmt, float deform_angle, float deform_scale, float deform_shift)
{
	...
	if (!dsfmt)
	{
		ccv_dense_matrix_t roi = ccv_dense_matrix(ENDORSE(box.rect.height), ENDORSE(box.rect.width), CCV_GET_DATA_TYPE(a->type) | CCV_GET_CHANNEL(a->type), ccv_get_dense_matrix_cell(a, ENDORSE(box.rect.y), ENDORSE(box.rect.x), 0), 0);
		roi.step = a->step;
		ccv_ferns_feature(ferns, &roi, ENDORSE(box.classification.id), fern);
	} else {
		...
		float m02 = (deform_shift * 2 * dsfmt_genrand_close_open(dsfmt) - deform_shift) * ENDORSE(box.rect.width);
		...
		float m12 = (deform_shift * dsfmt_genrand_close_open(dsfmt) - deform_shift) * ENDORSE(box.rect.height);
		...
		ccv_decimal_point_t p00 = ccv_perspective_transform_apply(ccv_decimal_point(0, 0), ccv_size(ENDORSE(box.rect.width), ENDORSE(box.rect.height)), m00, m01, m02, m10, m11, m12, m20, m21, m22);
		ccv_decimal_point_t p01 = ccv_perspective_transform_apply(ccv_decimal_point(ENDORSE(box.rect.width), 0), ccv_size(ENDORSE(box.rect.width), ENDORSE(box.rect.height)), m00, m01, m02, m10, m11, m12, m20, m21, m22);
		ccv_decimal_point_t p10 = ccv_perspective_transform_apply(ccv_decimal_point(0, ENDORSE(box.rect.height)), ccv_size(ENDORSE(box.rect.width), ENDORSE(box.rect.height)), m00, m01, m02, m10, m11, m12, m20, m21, m22);
		ccv_decimal_point_t p11 = ccv_perspective_transform_apply(ccv_decimal_point(ENDORSE(box.rect.width), ENDORSE(box.rect.height)), ccv_size(ENDORSE(box.rect.width), ENDORSE(box.rect.height)), m00, m01, m02, m10, m11, m12, m20, m21, m22);
		int padding_top = (int)(ccv_max(0, -ccv_min(p00.y, p01.y)) + 0.5) + 5;
		padding_top = ENDORSE(box.rect.y) - ccv_max(ENDORSE(box.rect.y) - padding_top, 0);
		int padding_right = (int)(ccv_max(0, ccv_max(p01.x, p11.x) - ENDORSE(box.rect.width)) + 0.5) + 5;
		padding_right = ccv_min(ENDORSE(box.rect.x + box.rect.width) + padding_right, a->cols) - ENDORSE(box.rect.x + box.rect.width);
		int padding_bottom = (int)(ccv_max(0, ccv_max(p10.y, p11.y) - ENDORSE(box.rect.height)) + 0.5) + 5;
		padding_bottom = ccv_min(ENDORSE(box.rect.y + box.rect.height) + padding_bottom, a->rows) - ENDORSE(box.rect.y + box.rect.height);
		int padding_left = (int)(ccv_max(0, -ccv_min(p00.x, p10.x)) + 0.5) + 5;
		padding_left = ENDORSE(box.rect.x) - ccv_max(ENDORSE(box.rect.x) - padding_left, 0);
		ccv_rect_t hull = ccv_rect(ENDORSE(box.rect.x) - padding_left, ENDORSE(box.rect.y) - padding_top,
								   ENDORSE(box.rect.width) + padding_left + padding_right,
								   ENDORSE(box.rect.height) + padding_top + padding_bottom);
		...
		ccv_dense_matrix_t roi = ccv_dense_matrix(ENDORSE(hull.height), ENDORSE(hull.width), CCV_GET_DATA_TYPE(a->type) | CCV_GET_CHANNEL(a->type), ccv_get_dense_matrix_cell(a, ENDORSE(hull.y), ENDORSE(hull.x), 0), 0);
		roi.step = a->step;
		ccv_dense_matrix_t* b = 0;
		ccv_perspective_transform(&roi, &b, 0, m00, m01, m02, m10, m11, m12, m20, m21, m22);
		roi = ccv_dense_matrix(ENDORSE(box.rect.height), ENDORSE(box.rect.width), CCV_GET_DATA_TYPE(b->type) | CCV_GET_CHANNEL(b->type), ccv_get_dense_matrix_cell(b, padding_top, padding_left, 0), 0);
		roi.step = b->step;
		ccv_ferns_feature(ferns, &roi, ENDORSE(box.classification.id), fern);
		ccv_matrix_free(b);
	}
...
{
	if (ENDORSE(box.width) == tld->patch.width && ENDORSE(box.height) == tld->patch.height)
		ccv_slice(a, (ccv_matrix_t**)b, type, ENDORSE(box.y), ENDORSE(box.x), ENDORSE(box.height), ENDORSE(box.width));
	else {
		assert((ENDORSE(box.width) >= tld->patch.width && ENDORSE(box.height) >= tld->patch.height) ||
			   (ENDORSE(box.width) <= tld->patch.width && ENDORSE(box.height) <= tld->patch.height));
		ccv_dense_matrix_t* c = 0;
		ccv_slice(a, (ccv_matrix_t**)&c, type, ENDORSE(box.y), ENDORSE(box.x), ENDORSE(box.height), ENDORSE(box.width));
...
static double _ccv_tld_box_variance(ccv_dense_matrix_t* sat, ccv_dense_matrix_t* sqsat, ccv_rect_t box)
{
	...
	double mean = (double)(brs - trs - bls + tls) / ENDORSE(box.width * box.height);
	...
	double variance = (double)(brsq - trsq - blsq + tlsq) / ENDORSE(box.width * box.height);
...
ccv_tld_t* ccv_tld_new(ccv_dense_matrix_t* a, ccv_rect_t box, ccv_tld_param_t params)
{
	...
	ccv_size_t patch = ccv_size((int)(sqrtf(params.min_win * params.min_win * (float)ENDORSE(box.width / box.height)) + 0.5),
								(int)(sqrtf(params.min_win * params.min_win * (float)ENDORSE(box.height / box.width)) + 0.5));
	ccv_array_t* good = 0;
	ccv_array_t* bad = 0;
	ccv_comp_t best_box = _ccv_tld_generate_box_for(ccv_size(a->cols, a->rows), patch, box, 20, &good, &bad, params);
	ccv_tld_t* tld = (ccv_tld_t*)ccmalloc(sizeof(ccv_tld_t) + sizeof(uint32_t) * (params.structs * ENDORSE(best_box.neighbors) - 1));
	tld->patch = patch;
	tld->params = params;
...
{
	ccv_dense_matrix_t* b = 0;
	float scale = sqrtf((float)ENDORSE(dd.rect.width * dd.rect.height) / (tld->patch.width * tld->patch.height));
	...
	dd.rect = ccv_rect((int)(dd.rect.x + (dd.rect.width - tld->patch.width * scale) + 0.5),
					   (int)(ENDORSE(dd.rect.y) + (ENDORSE(dd.rect.height) - tld->patch.height * scale) + 0.5),
		...
		for (i = 0; i < 2; i++) // run it twice to take into account the cases we missed when warm up
		{
			...
			for (j = 0; j < badex + good->rnum; j++)
			{
				...
				if (k < badex)
				{
					...
					{
						assert(box->neighbors >= 0 && box->neighbors < best_box.neighbors);
						if (ENDORSE(box->neighbors) % r1 == r0 &&
...
static ccv_array_t* _ccv_tld_long_term_detect(ccv_tld_t* tld, ccv_dense_matrix_t* ga, ccv_dense_matrix_t* sat, ccv_dense_matrix_t* sqsat, ccv_tld_info_t* info)
{
	...
	for_each_box(box, tld->patch.width, tld->patch.height, tld->params.interval, tld->params.shift, ga->cols, ga->rows)
			...
			_ccv_tld_box_variance(sat, sqsat, box.rect) > tld->var_thres)
		{
			...
			if (ENDORSE(box.classification.confidence) > tld->ferns_thres)
			{
				if (tld->top->rnum < tld->params.top_n)
				{
					...
					if (ENDORSE(top_box->classification.confidence < box.classification.confidence))
					{
...
ccv_comp_t ccv_tld_track_object(ccv_tld_t* tld, ccv_dense_matrix_t* a, ccv_dense_matrix_t* b, ccv_tld_info_t* info)
{
	...
	if (tld->found)
	{
		...
		{
			float scale = sqrtf((float)ENDORSE(result.rect.width * result.rect.height) / (tld->patch.width * tld->patch.height));
			// regularize the rect to conform patch's aspect ratio
			...
			ccv_matrix_free(c);
			if (ENDORSE(result.classification.confidence) > tld->nnc_verify_thres)
				verified = 1;
		}
	}
	if (info)
	...
	if (tracked)
	{
		if (dd->rnum > 0)
		{
			...
			for (i = 0; i < dd->rnum; i++)
			{
				...
				if (_ccv_tld_rect_intersect(result.rect, comp->rect) < 0.5 && ENDORSE(comp->classification.confidence > result.classification.confidence))
				{
					++confident_matches;
					ddcomp = comp;
				}
