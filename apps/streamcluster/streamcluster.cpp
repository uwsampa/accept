...
typedef struct {
  ...
  APPROX float cost;  /* cost of that assignment, weight*distance */
} Point;
...
APPROX float pspeedy(Points *points, APPROX float z, long *kcenter, int pid, pthread_barrier_t* barrier)
{
  ...
  for( int k = k1; k < k2; k++ )    {
    APPROX float distance = dist(points->p[k],points->p[0],points->dim);
  ...
  if( pid != 0 ) { // we are not the master threads. we wait until a center is opened.
    while(1) {
      ...
      for( int k = k1; k < k2; k++ )
	{
	  APPROX float distance = dist(points->p[i],points->p[k],points->dim);
	  if(ENDORSE( distance*points->p[k].weight < points->p[k].cost ))
	    {
	      points->p[k].cost = distance * points->p[k].weight;
	      points->p[k].assign=i;
	    }
    ...
    for(i = 1; i < points->num; i++ )  {
      APPROX bool to_open = ((float)lrand48()/(float)INT_MAX)<(points->p[i].cost/z);
      if(ENDORSE( to_open ))  {
	(*kcenter)++;
#ifdef ENABLE_THREADS
	pthread_mutex_lock(&mutex);
  ...
  */
  ...
  for ( i = k1; i < k2; i++ ) {
    APPROX float x_cost = dist(points->p[i], points->p[x], points->dim) 
      * points->p[i].weight;
    APPROX float current_cost = points->p[i].cost;

    if (ENDORSE( x_cost < current_cost )) {
...
int selectfeasible_fast(Points *points, int **feasible, int kmin, int pid, pthread_barrier_t* barrier)
{
  ...
  *feasible = (int *)malloc(numfeasible*sizeof(int));
  ...
  APPROX float* accumweight;
  APPROX float totalweight;
  ...
  for(int i=k1; i<k2; i++ ) {
    ...
    l=0;
    r=points->num-1;
    if(ENDORSE( accumweight[0] > w ))  { 
      (*feasible)[i]=0; 
      continue;
    }
    while( l+1 < r ) {
      k = (l+r)/2;
      if(ENDORSE( accumweight[k] > w )) {
	r = k;
      } 
      else {
	l=k;
