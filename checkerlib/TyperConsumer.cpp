...
void TyperConsumer::reportError(clang::Decl* decl,
                                llvm::StringRef message) {
  clang::DiagnosticsEngine &deng = ci.getDiagnostics();
  unsigned did = deng.getCustomDiagID(clang::DiagnosticsEngine::Error, message);
  clang::DiagnosticBuilder db = deng.Report(decl->getLocStart(), did);
  db.AddSourceRange(clang::CharSourceRange(decl->getSourceRange(), false));
}

void TyperConsumer::HandleTranslationUnit(clang::ASTContext &Ctx) {
